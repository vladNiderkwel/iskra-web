...
const deleteEvent = async () => {
    deleting.value = await useFetch(
        `${config.public.baseUrl}/event/${id}`, {
        method: "delete"
    })
}

watch(deleting, (value) => {
    if (value.status === "success") navigateTo("/event")
})
</script>
    ...
    <div class="w-full h-full max-h-full p-2 overflow-y-auto">
        ...
        <template v-else>
            ...
            <div class="w-1/2 mx-auto">
                ...
                <p class="mt-3 text-center">{{ getDateString(data.startDate) + " / " + getDateString(data.endDate) }}
                </p>
                ...
                <div class="flex self-center mt-3 w-fit">
                    <img v-if="data.author.photoUrl.length < 64" class="mr-2 w-6 h-6 rounded-full my-auto"
                        src="~assets/images/photo_placeholder.png" />
                    <img v-else class="mr-2 w-6 h-6 rounded-full my-auto"
                ...
                <ButtonFilledSecondary text="Удалить" @click="deleteEvent()" class="ml-auto mt-4">
                    <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
                        <path
