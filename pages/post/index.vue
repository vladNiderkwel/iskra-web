...
const { data, error, pending, refresh } =
    await useFetch(`${config.public.baseUrl}/post`, {
        method: 'get',
        query: {
    ...
    <div class="w-min mx-auto">
        ...
        <template v-else>
            ...
            <div class="flex mt-4 justify-center items-center pages" v-if="data.totalPages > 1">
                <NuxtLink v-if="data.currentPage >= 2" :to="`/post?page=${page - 1}`" class="page-number">
                    <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
                ...
                <NuxtLink class="page-number" v-for="page in data.totalPages"
                    :class="{ 'active-page': page === data.currentPage }" :to="`/post?page=${page}`">
                    <p>{{ page }}</p>
                </NuxtLink>

                <NuxtLink v-if="data.currentPage <= data.totalPages - 1" :to="`/post?page=${page + 1}`"
                    class="page-number">
                    <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
