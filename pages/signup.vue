...
const signup = async () => {
    ...
    signupFetch.value = await useFetch(`${config.public.baseUrl}/staff/signup`, {
        method: "post",
        body: {
...
<template>
    <div class="flex justify-between flex-col h-full">
        <form class="mx-auto flex flex-col items-center w-auto my-auto max-w-20" @submit.prevent="signup">

            <Title text="Первый вход в систему" />

            <div class="mt-8 flex w-full">
                <svg class="mr-3 my-auto" xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960"
                    width="24">
                    <path
            ...
            <div class="mt-4 flex w-full">
                ...
                <input type="password" v-model="oldPassword" placeholder="Выданный пароль" required
            ...
            <div class="mt-4 flex w-full">
                ...
                <input type="password" v-model="newPasswordConfirm" placeholder="Повторите новый пароль" required
                    class="text-input w-full">
            </div>
            ...
            <ErrorLabel message="Подобного пользователя нет" class="mx-auto mt-4"
                v-if="signupFetch.error && signupFetch.error.statusCode == 404" />

            <ErrorLabel message="Неверный email или пароль" class="mx-auto mt-4"
                v-else-if="signupFetch.error && signupFetch.error.statusCode == 401" />

            <LoadingIndicator v-else-if="signupFetch.pending == true" />
        </form>
